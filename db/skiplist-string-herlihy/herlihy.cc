...
fraser_insert(sl_intset_t *set, strkey_t key, strval_t val) 
{
 ...
 retry: 	
  ...
  new_node = sl_new_simple_node(key, val, get_rand_level(), 0);
  ...
  if (!ATOMIC_CAS_MB(&preds[0]->next[0], GET_UNMARKED(succs[0]), new_node))
    {
